...
│   ├── creditcard.csv             # ⬇️ Download from Kaggle (not in repo)
...
Download `creditcard.csv` from [Kaggle](https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud) and place it inside the `exercise/` folder.
...
### 4. Run the Notebook
Open `cc_fraud_detection.ipynb` in Jupyter Notebook or VS Code and **run all cells**.
This will train the models and automatically save them to the `models/` folder:
- `models/fraud_model.pkl` — Trained Neural Network
- `models/rbscaler.pkl` — Fitted RobustScaler
...
> ⚠️ The dataset (`creditcard.csv`, ~150MB) is **not included** in this repo. Download it from the Kaggle link above and place it in the `exercise/` folder before running the notebook.
>
> The trained models (`models/`) are also **not included** — they are automatically generated when you run the notebook.
